...
# 另一个终端实时查看:
tcpdump -i eth0 -XX | grep -E '0x2222|0x1111'
...
```bash
# 停止抓包后分析
tcpdump -r /tmp/cr_forwarding.pcap 'ether proto 0x2222' | head -20
# 预期看到多条:
#   TS_MAC > CR-2_MAC, ethertype RID (0x2222), length 80
#   含 payload: PROBE:TS->CR-2:seq=1

# 确认 RID 包结构正确 (24字节包头)
tcpdump -r /tmp/cr_forwarding.pcap 'ether proto 0x2222' -XX | head -40
```

**预期结果:**
- ✅ 抓包可见 EtherType=0x2222 的 RID 帧
...
# 实时过滤控制信令:
tcpdump -i <管理VLAN镜像口> 'ether proto 0x2222' -XX | grep -A5 'AUTH'
...
**抓包验证:**
```bash
tcpdump -r /tmp/auth.pcap 'ether proto 0x2222' -XX | grep -B2 -A10 'AUTH'
# 预期: EtherType=0x2222, DataType=0x01, payload 含 AuthRequest/Response
```
...
- ✅ 快速认证: AP 缓存命中, 不经过 CS
- ✅ 控制信令使用 EtherType=0x2222 + DataType=0x01
...
**抓包验证:**
```bash
# 1. 接入面: 应只看到 AID 帧 (0x1111)
tcpdump -r /tmp/access.pcap 'ether proto 0x1111' | wc -l
tcpdump -r /tmp/access.pcap 'ether proto 0x1111' -XX | head -60

# 2. 核心面: 应只看到 RID 帧 (0x2222)
tcpdump -r /tmp/core.pcap 'ether proto 0x2222' | wc -l
tcpdump -r /tmp/core.pcap 'ether proto 0x2222' -XX | head -40

# 3. 确认格式隔离
tcpdump -r /tmp/access.pcap 'ether proto 0x2222' | wc -l  # 预期: 0
tcpdump -r /tmp/core.pcap 'ether proto 0x1111' | wc -l      # 预期: 0

# 4. 跨 CR 通信
# Host-1 → Host-2 (修改 target_aid 为 Host-2 的 AID)
tcpdump -r /tmp/core.pcap 'ether proto 0x2222 and ether dst <CR-2_MAC>' | head
```

**预期结果:**
- ✅ 接入面 (VLAN 20): 仅见 AID 帧 (EtherType=0x1111)
- ✅ 核心面 (VLAN 10): 仅见 RID 帧 (EtherType=0x2222)
...
```bash
# 核心面: 同一AID包出现两次
tcpdump -r /tmp/mobility_core.pcap 'ether proto 0x2222' -XX | grep -B2 -A10 'MOVED'
# 第一次: CR-2 → CR-1 (远端旧映射)
# 第二次: CR-1 → CR-2 (CR-1重定向)

# 管理面: MobilityAlert 控制信令
tcpdump -r /tmp/mobility_mgmt.pcap 'ether proto 0x2222' -XX | grep -A5 'mobility'
```
...
```bash
# 过滤 AID 帧
tcpdump -r /tmp/access.pcap 'ether proto 0x1111'

# 过滤 RID 帧
tcpdump -r /tmp/core.pcap 'ether proto 0x2222'

# 过滤 RID 控制信令
tcpdump -r /tmp/core.pcap 'ether proto 0x2222 and ether[20:1]=0x01'
```
