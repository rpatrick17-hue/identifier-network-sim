...
def verify_rid_packet_structure():
    ...
    checks2 = [
        ("EtherType = 0x88B6 (RID)",   frame.ethertype == ETHERTYPE_RID),
...
async def verify_rid_encapsulation_forwarding():
    ...
    cr2.add_access_neighbor(host_aid, "00:11:22:33:44:02", 1)
    cr_add_remote_mapping(cr1.tables, AID.from_hex("969eef6ecad3c29a3a629280e686cf8d"),
                          RID(10002, 36192), cr2.my_rid, 100)
    ...
    print("  Host-1 发送 HTTP → AID 封装 → RID 转发 → TS")
    for i in range(3):
        await host.http_get(f"/page_{i}.html", ts.aid)
        await asyncio.sleep(0.1)

    m_cr1 = cr1.metrics.summary()
    m_cr2 = cr2.metrics.summary()
    m_ts = ts.metrics.summary()
    m_host = host.metrics.summary()

    print(f"\n  Host-1 发送: {m_host['sent_packets']} 包")
    print(f"  CR-1 收发:  {m_cr1['sent_packets']} 发送 / {m_cr1['recv_packets']} 接收")
    print(f"  CR-2 收发:  {m_cr2['sent_packets']} 发送 / {m_cr2['recv_packets']} 接收")
    print(f"  TS 接收:    {m_ts['recv_packets']} 包")

    ok = m_ts["recv_packets"] >= 3
    print(f"\n  RID 封装+转发+解封装 端到端: {'✅ TS收到全部请求' if ok else '❌'}")
