...
#   data_type(8b)              → 1B
#   ttl(8b)                    → 1B
#   dst_rid(64b)               → 8B (Q = unsigned long long)
#   src_rid(64b)               → 8B
...
def pack_rid_header(
    ...
    """Pack a RID header (24 bytes)."""
    ver_id = (version << 4) | id_type
    return RID_HEADER_STRUCT.pack(
        ver_id, qos_class, space_id, payload_length, data_type, ttl, dst_rid_int, src_rid_int
    )


def unpack_rid_header(data: bytes) -> dict:
    """Unpack a RID header, returning a dict of fields."""
    ver_id, qos, sid, plen, dtype, ttl, dst, src = RID_HEADER_STRUCT.unpack(data)
    return {
        "version": (ver_id >> 4) & 0xF,
        "id_type": ver_id & 0xF,
        "qos_class": qos,
